...
const vitePluginGraphQL = (options: Record<string, string>): Plugin => {
  return {
    name: 'graphql',
    transform(code, id) {
      if (!id.match('.gql')) {
      ...
      const doc = gql`
        ${code}
      `;
      const headerCode = `var doc = ${JSON.stringify(doc)};
      doc.loc.source = ${JSON.stringify(doc.loc.source)};`;

      let outputCode = '';

      const operationCount = doc.definitions.reduce(function (accum, op) {
      ...
      return { code: allCode, map: null };
    },
  };
};

function expandImports(source: string, doc: DocumentNode) {
  const lines = source.split(/\r\n|\r|\n/);
  let outputCode = `var names = {};
